...
class _SimulatorsSection extends ConsumerWidget {
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final sims = [
      _SimDef(
        ...
        label: 'Obstacle',
        sublabel: 'Inject obstacle event',
        color: AppColors.warning,
        event: DebugEvent.obstacle,
      ),
      _SimDef(
        icon: Icons.battery_alert_rounded,
        label: 'Low Battery',
        sublabel: 'Drop battery to 10%',
        color: AppColors.batteryLow,
        event: DebugEvent.lowBattery,
      ),
      _SimDef(
        icon: Icons.credit_card_off_rounded,
        label: 'RFID Fail',
        sublabel: 'Trigger RFID failure',
        color: AppColors.danger,
        event: DebugEvent.rfidFail,
      ),
      _SimDef(
        icon: Icons.inventory_2_rounded,
        label: 'Out of Stock',
        sublabel: 'Simulate stock depletion',
        color: AppColors.info,
        event: DebugEvent.outOfStock,
      ),
    ];
...
class _SimButton extends StatefulWidget {
  const _SimButton({
    required this.def,
  ...
  @override
  State<_SimButton> createState() => _SimButtonState();
}

class _SimButtonState extends State<_SimButton> {
  bool _hovered = false;

  @override
  Widget build(BuildContext context) {
    final c = widget.def.color;
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: Icon(widget.def.icon, color: c, size: 15),
                  ),
                  const Spacer(),
                  if (widget.loading)
                    SizedBox(
